...
def wolfeCondition(fun, d, x, fx, gx, g, c1=1e-4, rho=0.5, alpha_in=1.0, maxback=30):
    alpha = alpha_in
    q = np.dot(gx, d)
    j = 0
...
def steepest_descent(fun, x, gradfun, tol=1e-10, maxit=1000):
    ...
    while grad_norm>tol and it<=maxit:
        ...
        alpha = wolfeCondition(fun,d,x,fx,-d, gradfun)
        x = x + alpha*d
        grad_norm = np.linalg.norm(gradfun(x))
        it = it + 1
...
def polynomial_armijo(fun, d, x, fx, g, gx, c1=1e-4, rholo=0.1, rhohi=0.5, alpha_in=1.0, tol=1e-10, maxpol=30):
    """
	Parameters
	----------
		d:array

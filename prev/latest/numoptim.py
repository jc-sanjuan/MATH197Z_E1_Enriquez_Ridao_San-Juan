...
Name: Raphaell Ridao, Jan Catherine San Juan, Deangelo Enriquez
...
def strongWolfe_conditions(fun, d, x, fx, gx, g, c1=1e-4, rho=0.5, alpha_in=1.0, maxback=30):
    ...
    while fun(x+alpha*d)>fx+c1*alpha*q or abs(np.dot(g(x + alpha*d), d)>0.9*q) and j<= maxback:
        alpha = rho*alpha
        j = j+1 
    print("maxbackiteration		: {}" .format(j))
    return alpha


def goldstein_conditions(fun, d, x, fx, gx, g, rho=0.5, alpha_in=1.0, maxback=30):
    j = 0
    alpha = alpha_in
    q = np.dot(gx, d)
    #fx1 = fun(x + alpha*d)
    goldstein_condition1 = fx + 0.75*alpha*q < fun(x + alpha*d)
    goldstein_condition2 = fun(x + alpha*d) > fx + 0.25*alpha*q
    goldstein = goldstein_condition1 or goldstein_condition2
    f1 = fx + 0.75*alpha*q
    f2 = fx + 0.25*alpha*q
    #while j < maxback:
    while goldstein and  j < maxback:
    #while fun(x + alpha*d) > (fx + 0.25*alpha*q) and  j < maxback:
    #while ((fun(x + alpha*d) > (fx + 0.25*alpha*q)) or (fun(x + alpha*d) < (fx + (0.75*alpha*q)))) and  j < maxback:
        #if not(fun(x + alpha*d) >= (fx + (0.75*alpha*q)) and (fun(x + alpha*d) <= (fx + 0.25*alpha*q))):
            #if (fx + (0.25*alpha*q) >= fun(x + alpha*d)):
                #alpha = rho*alpha
                #print("maxback		: {}" .format(grad_norm))
        alpha = rho*alpha
        
        goldstein_condition1 = fx + 0.75*alpha*q > fun(x + alpha*d)
        goldstein_condition2 = fun(x + alpha*d) > fx + 0.25*alpha*q
        goldstein = goldstein_condition1 or goldstein_condition2
        print("condition 1		: {}" .format(goldstein_condition1))
        print("condition 2		: {}" .format(goldstein_condition2))
...
def steepest_descent(fun, x, gradfun, tol=1e-10, maxit=50000):
    ...
    while grad_norm>tol and it<=maxit:
        ...
        if operator == 'Armijo' or operator == '1':
            ...
            alpha = goldstein_conditions(fun,d,x,fx,-d, gradfun)
...
def polynomial_goldstein(fun, d, x, fx, gx, g, c1=1e-4, rholo=0.1, rhohi=0.5, alpha_in=1.0, tol=1e-10, maxpol=30):
   ...
   # while fj > fx + 0.25*alpha*q or (fj >= fx + 0.75*alpha*q and j <= maxpol):
        if j == 0:
            alphastar = ((-q)*alpha**2)/(2*(fj-fx-(alpha*q)))
